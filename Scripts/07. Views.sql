...
Select * exclude(C_ADDRESS, C_PHONE) from SNOWFLAKE_SAMPLE_DATA.TPCH_SF1.CUSTOMER;

Select * from v_col_customer;

-- View based on Multiple Tables
Create or replace view view_multiple_tables 
AS
    Select  
        N.N_Name,
        COUNT(C.C_CUSTKEY) CUSTOMER_COUNT 
...
Select C_CUSTKEY, C_NAME, C_NATIONKEY, C_PHONE, C_ACCTBAL, C_MKTSEGMENT, C_COMMENT from SNOWFLAKE_SAMPLE_DATA.TPCH_SF1.CUSTOMER;
...
-- Below will Not work
Create or replace materialized view matv_customer 
AS
    Select  
        N.N_Name,
        COUNT(C.C_CUSTKEY) 
...
SELECT GET_DDL('VIEW', 'matv_customer');
